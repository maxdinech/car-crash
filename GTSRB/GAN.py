...
images = np.load('data/train/train_clahe.npy')
labels = np.load('data/train/train_labels.npy')
images = np.array([images[i] for i in range(len(images)) if labels[i,25] == 1])
...
G = Generator(30, 100, 40*40)  # Autant de sorties que d'entrées dans D
D = Discriminator(40*40, 100, 30, 1)  # Une seule sortie : classification binaire
...
def acc_fn(y_pred, y):
    return 100 * ones(len(y))[(y_pred - y).abs() < 0.5].sum() / len(y)
...
def entropy(n):
    return Variable(torch.randn(n, G.input_dim).type(dtype), requires_grad=False)
...
def ascii_print_sided(img1, img2):
    img1 = img1.view(40,40)
    sep = torch.zeros(40,2)
    img2 = img2.view(40,40)
    image = torch.cat((img1, sep, img2), dim = 1)
    for ligne in image:
        for pix in ligne:
            print(2*" ░▒▓█"[int(pix*4.999)%5], end='')
        print('')
...
for e in range(epochs):
    ...
    for i in range(0, len(images) - batch_size + 1, batch_size):
        ...
        loss = loss_fn(D_pred, ones(batch_size))
        loss.backward()
        ...
        for param in D.parameters():
            param.data -= eta_D * param.grad.data
        ...
        for _ in range(5):
            ...
            for param in G.parameters():
                param.data -= eta_G * param.grad.data
    ...
    D_loss_real = loss_fn(D_pred_real, ones(len(images)))
    D_loss_fake = loss_fn(D_pred_fake, zeros(len(images)))
    D_loss = (D_loss_real + D_loss_fake).data[0] / 2

    D_acc_real = acc_fn(D_pred_real, ones(len(images)))
    D_acc_fake = acc_fn(D_pred_fake, zeros(len(images)))
    D_acc = (D_acc_real + D_acc_fake).data[0] / 2

    G_loss = loss_fn(D_pred_fake, ones(len(images))).data[0]
    G_acc = acc_fn(D_pred_fake, ones(len(images))).data[0]
